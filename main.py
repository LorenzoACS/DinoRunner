...
def displayStore():
	...
	title = pygame.font.SysFont(fontType, fontSize)
	titleText = title.render('Store', True, pygame.Color('white'))
	titleRect = titleText.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//10))
	exitStoreFont = pygame.font.SysFont(fontType, fontSize//2)
	exitStoreText = exitStoreFont.render('X' , True , pygame.Color('white'))
	buyDinoF = pygame.font.SysFont(fontType, fontSize//3)
	buyDinoT = buyDinoF.render('Buy another dino! Current number of dinos: ' + str(dinoCounter), True, pygame.Color('white'))
	buyDinoR = buyDinoT.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//4))
	dinoButton = buyDinoF.render('Cost: ' + str(dinoCost) , True , pygame.Color('white'))
...
def makeCoin():
	...
	global activeCoin
	activeCoin = True
	coinX = random.randint(1300, 1500)
	coin = Coin((coinX, dinoY))
...
def main():
	...
	while run:  
		...
		if not activeStore and not gameActive:
			homeScreen()
		elif not activeStore and gameActive:
			gameScreen()
			...
			if not activeCoin:
				...
				coinFrames()
			screen.blit(coin.image, (coin.rect.x, dinoY))
		elif activeStore:
			displayStore()
				...
				if SCREEN_WIDTH - 256 <= mouse[0] <= SCREEN_WIDTH and 0 <= mouse[1] <= 70:
					if gameActive:
						if activeStore:
							...
							activeStore = True
							displayStore()
				if activeStore and score - dinoCost >= 0 and (SCREEN_WIDTH // 2) - 128 <= mouse[0] <= (SCREEN_WIDTH // 2) + 128 and SCREEN_HEIGHT // 3 <= mouse[1] <= (SCREEN_HEIGHT // 3) + 80:
					score -= dinoCost
					dinoCounter += 1
		pygame.display.flip()
	pygame.quit()
